...
- [x] **Step 4: README.** Ch10 block after the Ch9 block (one bullet for 10.1 naming the three challenges, one for 10.d); bump the counter line to **"Nine chapters live — sixteen interactive labs."**
- [x] **Step 5: DESIGN_PLAN.** Append a progress note to the Phase 5 paragraph: *(in progress: ch11 brokers shipped earlier; ch10 shipped 2026-07-17 — 10.1 MapReduce-vs-dataflow twin lab + 10.d debrief; the §4 row-10 win condition [kill a worker mid-job] is challenge 1, and the barrier/pipelining recovery contrast is engine-verified by the property suite.)*
- [x] **Step 6: Gate + commit.** (commits 6c99b97 debrief, 8b994f4 dot-key fix, ed1fa42 wiring; browser-verified 10.1 healthy+kill-recovery & 10.d, console clean) `npx vitest run && npx tsc -b && npm run build`.

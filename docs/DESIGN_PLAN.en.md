...
**Phase 3 — Distributed core: Chapters 6, 8, 9 (4–6 weeks).** Partitioning ring → unreliable network → Raft + linearizability checker. The hardest and most valuable cluster. *(Phase 3 COMPLETE 2026-07-16 — ch6, ch8, ch9 all shipped. ch8: 8.1 lease/fencing playground + 8.d debrief; fig 8-4 GC-pause corruption, fencing rejection, and clock-skew corruption all engine-verified. ch9: 9.1 five-node Raft + Wing–Gong linearizability checker + 9.d debrief; minority silence, heal truncation, and the deposed-leader stale read all engine-verified, checker cross-checked by property suite.)*
...
**Phase 5 — Data flow: Chapters 10, 11, 12 + Chapters 1, 2, 4 (4 weeks).** Finish with the "unbundled database" lab composing everything together. *(in progress: ch11 brokers shipped earlier; ch10 shipped 2026-07-18 — 10.1 MapReduce-vs-dataflow twin lab + 10.d debrief; the §4 row-10 win condition [kill a worker mid-job] is challenge 1, and the barrier/pipelining recovery contrast is engine-verified by the property suite.)* *(ch12 shipped 2026-07-18 — 12.1 Unbundled Database: one write → append-only log → three lagging derived views [search index / cache / analytics]; single-node authoritative model; the three challenges are stale-read/RYW, rebuild-from-log, and exactly-once-via-offset-dedup — all engine-verified by the property + pinned-lesson suites. the unbundled-database capstone is done.)* *(ch1 shipped 2026-07-20 — 1.1 Load Simulator: requests through an M/M/c queue, live p50/p95/p99 vs an SLA line, with load / servers / cache / service-variance / fan-out knobs; the three challenges are the knee+replica-rescue, variance-drives-the-tail, and fan-out tail-latency amplification — all engine-verified by the property + pinned-lesson suites (server-count coupling, percentile ordering, response≥service) + 1.d debrief carrying Reliability & Maintainability as prose. Only ch2 remains in Phase 5.)* *(ch2 shipped 2026-07-20 — 2.1 Model Shape-Shifter: one social-graph dataset stored three ways [relational tables / denormalized documents / graph], the same query animated step-by-step in all three with a live round-trip count; challenges are the FoF join tax [document N+1 vs graph's one traversal], many-to-many [documents can't join], and schema flexibility [schema-on-read absorbs a new field free vs a relational migration] — all engine-verified by the property suite (identical result across models for any query+root) + pinned lesson + 2.d debrief. **Phase 5 and the whole DDIA book are now complete — all 12 chapters shipped.**)*
